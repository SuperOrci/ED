...
bool AD5933::reset() {
  ...
  // Send byte back
  bool result = sendByte(CTRL_REG2, val);
...
bool AD5933::setInternalClock(bool internal) {
  ...
  if (internal) {
    bool result = setClockSource(CLOCK_INTERNAL);
    Serial.print("setInternalClock Result : ");
    Serial.print(result);
    Serial.print("\n");
    return result;
  } else {
    bool result = setClockSource(CLOCK_EXTERNAL);
    Serial.print("setInternalClock Result : ");
    Serial.print(result);
    Serial.print("\n");
    return result;
...
bool AD5933::setStartFrequency(unsigned long start) {
  ...
  bool result = sendByte(START_FREQ_1, highByte) && sendByte(START_FREQ_2, midByte) && sendByte(START_FREQ_3, lowByte);
  Serial.print("setStartFrequency Result : ");
  Serial.print(result);
  Serial.print("\n");
}
...
bool AD5933::setIncrementFrequency(unsigned long increment) {
  ...
  bool result = sendByte(INC_FREQ_1, highByte) && sendByte(INC_FREQ_2, midByte) && sendByte(INC_FREQ_3, lowByte);
  Serial.print("setIncrementFrequency Result : ");
  Serial.print(result);
  Serial.print("\n");
  return result;
}
...
bool AD5933::setNumberIncrements(unsigned int num) {
  ...
  bool result = sendByte(NUM_INC_1, highByte) && sendByte(NUM_INC_2, lowByte);
  Serial.print("setNumberIncrements Result : ");
  Serial.print(result);
  Serial.print("\n");
  return result;
}
...
bool AD5933::setPGAGain(byte gain) {
  ...
  if (gain == PGA_GAIN_X1 || gain == 1) {
    ...
    val |= PGA_GAIN_X1;
    bool result = sendByte(CTRL_REG1, val);
    Serial.print("setPGAGain Result : ");
    Serial.print(result);
    Serial.print("\n");
...
bool AD5933::getComplexData(int *real, int *imag) {
  ...
  byte realComp[2];
  byte imagComp[2];
  if (getByte(REAL_DATA_1, &realComp[0]) && getByte(REAL_DATA_2, &realComp[1]) && getByte(IMAG_DATA_1, &imagComp[0]) && getByte(IMAG_DATA_2, &imagComp[1])) {
